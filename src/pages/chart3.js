...
const Chart3 = () => {
  ...
  const setXaxispre = (data, sortBy) => {
    ...
    temp1.map((item) => {
      let sum = 0;
      data.map((item2) => {
        let ind = item2.findIndex(
          (x) => x["L2 Cluster"] === item && x["Sentiment"] === "Positive"
        );
        if (ind >= 0) {
          sum += parseInt(item2[ind]["L2 Review Count"]);
        }
        let ind2 = item2.findIndex(
          (x) => x["L2 Cluster"] === item && x["Sentiment"] === "Negative"
        );
        if (ind2 >= 0) {
    ...
    <>
      ...
      <div className="app_container">
        <div style={{ display: "flex", marginBottom: "10px" }}>
          <div style={{ width: "40%" }}>
            <Select options={ops} onChange={handleChangeSort} />
          </div>
        </div>
